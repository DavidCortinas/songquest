...
from songquest.utilities.list_utilities import chunk_list
...
def search_song(request):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        bmi_future = executor.submit(bmi_scraper.get_bmi_results, song, performer)
    ...
    response_data = {"ascap_results": ascap_data, "bmi_results": bmi_data}
...
def get_user(request):
    ...
    try:
        ...
        return JsonResponse(
            {"error": "User does not exist", "isRegistered": False}, status=404
        )
...
def get_user_profile(request):
    ...
    try:
        ...
        return JsonResponse(
            {"error": "Profile does not exist for the user."}, status=404
        )
...
def discover_song(request):
    try:
        ...
        return JsonResponse(
            {"error": "Missing action parameter in request."}, status=400
        )
    ...
    try:
        ...
        if token_expired(expires_at):
            ...
            if token_info:
                ...
                return JsonResponse(
                    {"error": "Failed to refresh access token"}, status=400
                )
            ...
            "badge": (
                {
                    ...
                    "image_url": (
                        request.build_absolute_uri(achievement.badge_reward.image.url)
                        if achievement.badge_reward.image
                        else None
                    ),
                }
...
def create_playlist(request):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            return JsonResponse(
                {"error": "Failed to create playlist"}, status=response.status_code
            )
    ...
    try:
        ...
        for playlist_id in playlist_ids:
            ...
            if response.status_code in (200, 204):
                ...
                return JsonResponse(
                    {
                        "error": "Failed to unfollow playlist on Spotify",
...
def add_to_playlist(request, playlist_id):
    ...
    try:
        ...
        max_order = (
            PlaylistSong.objects.filter(playlist=playlist).aggregate(Max("order"))[
                "order__max"
            ]
            or -1
        )
        for track in tracks:
            ...
            PlaylistSong.objects.create(
                playlist=playlist, song=song, order=max_order, added_on=timezone.now()
            )
        ...
        try:
            ...
            print(
                "Error parsing or accessing snapshot_id from response:", response.text
            )
        ...
        if response.status_code == 201 or response.status_code == 200:
            ...
            return JsonResponse(
                {"error": "Failed to add to playlist"}, status=response.status_code
            )
...
def remove_from_playlist(request, playlist_id):
    ...
    try:
        ...
        body = json.dumps(
            {
                "tracks": [
                    {"uri": f'spotify:track:{track["spotifyId"]}' for track in tracks}
                ]
            }
        )
        ...
        for track in tracks:
            ...
            PlaylistSong.objects.filter(
                playlist=playlist, song=song, removed_on__isnull=True
            ).update(removed_on=timezone.now())
        ...
        if response.status_code == 200:
            ...
            return JsonResponse(
                {"error": "Failed to remove from playlist"}, status=response.status_code
            )
        ...
        if response.status_code in [200, 201]:
            ...
            with transaction.atomic():
                for index, track_id in enumerate(track_ids):
                    ...
                    PlaylistSong.objects.filter(playlist=playlist, song=song).update(
                        order=index
                    )
            ...
            ordered_tracks = [
                ...
                for pt in PlaylistSong.objects.filter(playlist=playlist)
                .select_related("song")
                .order_by("order")
            ]
...
def save_request_parameters(request):
    if request.method == "POST":
        try:
            ...
            if not user_id:
                return JsonResponse(
                    {"error": "User-Id not found in headers"}, status=400
                )
...
def add_to_spotify(request):
    if request.method == "POST":
        ...
        try:
            ...
            if response.status_code == 200:
                return JsonResponse(
                    {"message": "Added to Spotify library successfully"}, status=200
                )
            else:
...
def check_users_tracks(request):
    if request.method == "POST":
        ...
        try:
            ...
            track_ids = [
                track.get("spotifyId", track.get("id")) for track in recommendations
            ]
            all_responses = []
            ...
            return JsonResponse(all_responses, safe=False, status=200)
...
def remove_users_tracks(request):
    if request.method == "POST":
        ...
        try:
            ...
            response = requests.delete(spotify_url, headers=headers, params=params)

            if response.status_code == 200:
                return JsonResponse(
                    {"message": "Removed from Spotify library successfully"}, status=200
                )
            else:
    ...
    if request.method == "PUT":
        ...
        try:
            ...
            if response.status_code == 204:
                return JsonResponse(
                    {"message": "Artists followed successfully"}, status=204
                )
            else:
...
def check_if_user_follows_artists(request):
    if request.method == "GET":
        ...
        try:
            ...
            spotify_url = f"https://api.spotify.com/v1/me/following/contains"
            ...
            for chunk in chunk_list(artist_ids, 50):
                ...
                if response.status_code == 200:
                    ...
                    return JsonResponse(
                        {"error": "Failed to check if user follows artists"},
                        status=response.status_code,
                    )
...
def unfollow_artists(request):
    if request.method == "DELETE":
        ...
        try:
            ...
            )  # Make sure to decode the request body
            ...
            if response.status_code == 204:
                return JsonResponse(
                    {"message": "Successfully unfollowed the artists"}, status=204
                )
            else:
                return JsonResponse(
                    {"error": "Failed to unfollow artists"}, status=response.status_code
                )
