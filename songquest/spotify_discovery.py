...
def format_spotify_url(parameters):
    ...
    (
        add_param("target_acousticness", target_acousticness)
    ...
    (
        add_param("target_danceability", target_danceability)
    ...
    add_param("target_duration_ms", target_duration_ms) if target_duration_ms else None

    add_param("min_energy", min_energy) if min_energy else None
    add_param("max_energy", max_energy) if max_energy else None
    add_param("target_energy", target_energy) if target_energy else None

    (
        add_param("min_instrumentalness", min_instrumentalness)
        if min_instrumentalness
        else None
    )
    (
        add_param("max_instrumentalness", max_instrumentalness)
        if max_instrumentalness
        else None
    )
    (
        add_param("target_instrumentalness", target_instrumentalness)
        if target_instrumentalness
        else None
    )

    # add_param("min_key", min_key) if min_key else None
    # add_param("max_key", max_key) if max_key else None
    add_param("target_key", target_key) if target_key else None
    ...
    add_param("target_liveness", target_liveness) if target_liveness else None
    ...
    add_param("target_loudness", target_loudness) if target_loudness else None

    add_param("min_mode", min_mode) if min_mode else None
    add_param("max_mode", max_mode) if max_mode else None
    add_param("target_mode", target_mode) if target_mode else None
    ...
    add_param("target_popularity", target_popularity) if target_popularity else None
    ...
    add_param("target_speechiness", target_speechiness) if target_speechiness else None

    add_param("min_tempo", min_tempo) if min_tempo else None
    add_param("max_tempo", max_tempo) if max_tempo else None
    add_param("target_tempo", target_tempo) if target_tempo else None

    add_param("min_time_signature", min_time_signature) if min_time_signature else None
    add_param("max_time_signature", max_time_signature) if max_time_signature else None
    (
        add_param("target_time_signature", target_time_signature)
    ...
    add_param("target_valence", target_valence) if target_valence else None
