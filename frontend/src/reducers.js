	...
	case ADD_TO_PLAYLIST_TO_EDIT:
		if (editPlaylistIndex !== -1) {
			...
			const updatedPlaylist = {
				...
				tracks: [
					...state.playlists[editPlaylistIndex].tracks,
					...payload.tracks
				]
			};
	...
	case REMOVE_FROM_PLAYLIST_TO_EDIT:
		if (editPlaylistIndexRemove !== -1) {
			...
			const updatedPlaylist = {
				...
				tracks: state.playlists[editPlaylistIndexRemove].tracks.filter(track =>
					!payload.tracks.some(removeTrack => removeTrack.spotifyId === track.spotifyId)
				)
			};
			// Clone the playlists array and replace the edited playlist
			const updatedPlaylists = [
			...
			currentPlaylist: {
				...
				createPlaylist: {
					...
					tracks: state.currentPlaylist.createPlaylist.tracks.filter(song => 
						!payload.trackIds.includes(song.spotifyId)),
				},
			},
		};
